package com.aurionpro.jwtsecured.security;
...
//Token generation
//Token validation
//needs secret key and expiration date
...
public class JwtTokenProvider {
	...
	//username>subject,iat,expiration date>claims
	public String generateToken(Authentication authentication) {
		...
		String token= Jwts.builder().claims().subject(username).issuedAt(new Date(System.currentTimeMillis())).expiration(expireDate).and().signWith(key()).claim("role", authentication.getAuthorities()).compact();
	...
	//payload- has all claims > i need subject from payload
